...
int stop=-1,potop=-1,o=-1,printf_num=0;//o保持和c中符号一致就行 用来比较 
char now_func[STRINGLENGTH];
void four(int type,char d[],char s1[],char s2[])
{//$8,$9负责取操作数值出来,$10负责存结果，$11负责找结果地址
	...
	switch(type){
		...
		case(SCANF):
			...
			break;
			//sp是栈顶第一个元素、fp是栈底最后一个元素,v0返回值 fprintf(fa,"");
		case(CALL)://1、检查函数类型参数类型??????2、装载参数3、装载返回地址、返回值4、转移过程
			printf("CALL %s\n",dst);
			//strcpy(now_func,dst);
			fprintf(fa,"jal %s\n",dst);
			break;
